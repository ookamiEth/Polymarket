#!/usr/bin/env python3
"""
Batch fetch tick-by-tick CLOB trade data with CONSOLIDATION-FIRST architecture.

Streams data directly into consolidated parquet files (one per market category).
No individual files created - data is consolidated as it's collected.

Features:
- Thread-safe consolidated file writes (19 category files)
...
Usage:
    ...
    uv run python scripts/batch_fetch_clob_data_consolidated.py \
      ...
      --checkpoint-file checkpoints/consolidated_progress.jsonl
...
# Import consolidated writer modules
from market_categorizer import categorize_market
from consolidated_writer import ConsolidatedWriter
...
DEFAULT_OUTPUT_DIR = "data/clob_ticks_consolidated"
DEFAULT_WORKERS = 5
...
def save_to_consolidated(
    df: pl.DataFrame,
    market_info: Dict,
    writer: ConsolidatedWriter,
    verbose: bool = False
) -> tuple[bool, Optional[str]]:
    """
    Save DataFrame to consolidated parquet file.
    ...
    if not category:
        if verbose:
            print(f"[WARN] Could not categorize market: {slug}", file=sys.stderr, flush=True)
        return (False, None)

    success = writer.append_market(df, category, slug, verbose=verbose)
...
def collect_single_market(
    ...
    """
    Collect trades for a single market and append to consolidated file.
    ...
    try:
        ...
        success, category = save_to_consolidated(df_trades, market_info, writer, verbose=verbose)

        if not success:
            raise Exception(f"Failed to write to consolidated file (category: {category})")
...
def collect_all_markets(
    ...
    # Create consolidated writer (thread-safe)
    writer = ConsolidatedWriter(output_dir)
    print(f"   ✓ Consolidated writer initialized")
        ...
        with ThreadPoolExecutor(max_workers=workers) as executor:
            ...
            for market in markets:
                future = executor.submit(
                    ...
                    market,
                    rate_limiter,
                    writer,  # Pass consolidated writer
                    output_dir,
                    checkpoint_file,
                    verbose
                )
