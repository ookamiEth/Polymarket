#!/usr/bin/env python3
"""
Fetch all BTC 15-minute markets from Polymarket Gamma API

Queries the Polymarket Gamma API to get complete history of BTC up/down 15-minute markets
and saves them to a parquet file for analysis.

Usage:
    uv run python scripts/fetch_btc_15min_markets_from_api.py
    uv run python scripts/fetch_btc_15min_markets_from_api.py --output data/markets/btc_15min_all_markets.parquet
"""
...
DEFAULT_OUTPUT = 'data/markets/btc_15min_all_markets.parquet'

def fetch_btc_15min_markets(verbose: bool = False) -> list:
    """
    Fetch all BTC 15min markets from Gamma API with pagination

    Returns:
        List of market dictionaries
    """
    print('=== Fetching BTC 15min Markets from Polymarket Gamma API ===\n')
    ...
    while page <= max_pages:
        ...
        try:
            ...
            # Filter for BTC 15min markets
            btc_15min = [m for m in markets if 'btc-up-or-down-15m' in m.get('slug', '')]

            if verbose:
                print(f'  Received {len(markets)} markets, {len(btc_15min)} BTC 15min markets')

            all_markets.extend(btc_15min)
    ...
    print(f'\nTotal BTC 15min markets fetched: {len(all_markets)}')
    return all_markets
...
def analyze_markets(markets: list):
    ...
    print(f'\nTotal trading period: {duration_days:.1f} days')
    print(f'Expected periods (96/day): {duration_days * 96:.0f}')
    print(f'Actual periods found: {len(df)}')
    print(f'Coverage: {len(df) / (duration_days * 96) * 100:.1f}%')
...
def main():
    parser = argparse.ArgumentParser(description='Fetch BTC 15min markets from Polymarket API')
    parser.add_argument(
        '--output',
        type=str,
    ...
    # Fetch markets
    markets = fetch_btc_15min_markets(verbose=args.verbose)
