#!/usr/bin/env python3
"""
Generate 1 year schedule of Bitcoin Up or Down hourly market timestamps

Creates a parquet file with all hourly market periods in ET timezone.
Slug format: bitcoin-up-or-down-{month}-{day}-{hour}-et

Usage:
    cd /orderbook_snapshots
    uv run python scripts/generate_schedule_btc_hourly.py
"""
...
def generate_btc_hourly_schedule(
    start_timestamp: int = None,
    days: int = 365,
    interval_minutes: int = 60
) -> pl.DataFrame:
    """
    Generate schedule of Bitcoin Up or Down hourly markets
    ...
    print(f"Generating Bitcoin Up or Down hourly market schedule (ET timezone)")
    print(f"="*80)
    ...
    for i in range(total_periods):
        ...
        # Format: bitcoin-up-or-down-october-8-1am-et
...
def main():
    ...
    # Generate schedule
    df = generate_btc_hourly_schedule(
    ...
    # Save to parquet
    output_file = "config/btc_hourly_schedule_1year.parquet"
    print(f"\n{'='*80}")
