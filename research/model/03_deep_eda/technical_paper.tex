...
\text{Low} & \text{if } \text{RV}_{900s} \leq \text{median}(\text{RV}_{900s}) \\
...
\midrule
Dataset Extension & 2 hours & Statistical power & Low & 1 \\
Feature Pruning & 4 hours & +0.5-1.0\% Brier & Low & 2 \\
Advanced Moneyness & 4-5 hours & +6.5-10.5\% Brier & Low & 3 \\
Order Book Spreads & 2 hours & +1.0-2.0\% Brier & Low & 4 \\
Volatility Asymmetry & 3.5 hours & +2.6-5.1\% Brier & Low & 5 \\
Walk-Forward CV & 1 week & Robustness & Low & 6 \\
Weighted MSE & 1 day & +2-4\% Brier & Moderate & 7 \\
Regime Models & 1 week & +5-7\% Brier & Moderate & 8 \\
Confusion Matrix & 1 day & Trading insights & Low & 9 \\
Log-Odds Transform & 2-3 days & +1-2\% Brier & Moderate & 10 \\
Neural Network Ensemble & 1 week & +0.5-2.0\% Brier & High & 11 \\
\bottomrule
\end{tabular}
Implementation roadmap ordered by return-on-investment.\\\rule{\textwidth}{0.4pt}
\end{table}

\textbf{Recommended Phased Approach:}
\begin{enumerate}
\item \textbf{Phase 1 (Week 1-2)}: Dataset extension, feature pruning, advanced moneyness, order book spreads, volatility asymmetry
    \begin{itemize}
    \item Expected: 10-18\% total Brier improvement (0.1524 → 0.1350-0.1410)
    \item Final feature count: 175 (from 226)
    \item Training speed: +25\% faster
    \end{itemize}
\item \textbf{Phase 2 (Week 3-4)}: Walk-forward CV, regime models
    \begin{itemize}
    \item Expected: +5-7\% additional improvement
    \item 10 temporal folds with mean ± std reporting
    \item 4 regime-specific models
    \end{itemize}
\item \textbf{Phase 3 (Week 5)}: Confusion matrix analysis, trading insights
    \begin{itemize}
    \item Identify no-trade zones ([0.45-0.55])
    \item Position sizing via Kelly criterion
    \item Compare BS vs ML bucket-by-bucket
    \end{itemize}
\item \textbf{Phase 4 (Optional)}: Weighted MSE, log-odds transform, neural network ensemble
    \begin{itemize}
    \item Only if Phase 1-3 improvements validated
    \item Expected: +3-8\% additional improvement
    \end{itemize}
\end{enumerate}
