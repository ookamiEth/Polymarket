...
\textbf{Stage 2}: Train LightGBM gradient boosting model to predict residuals $\epsilon = \text{Outcome} - P_{\text{BS}}$ using 175 engineered features spanning market microstructure, volatility dynamics, momentum, order book depth, funding rates, and cyclical patterns.
...
\addcontentsline{toc}{section}{Paper Organization}

\noindent\textbf{Section 2 — Code and Implementation} \\
Lists all implementation files, data processing pipelines, and analysis scripts for full reproducibility.

\noindent\textbf{Section 3 — Mathematical Framework} \\
Establishes the theoretical foundation for two-stage residual learning. Presents Black-Scholes binary option baseline and proves that MSE of residuals directly equals Brier score improvement (Theorem 1).

\noindent\textbf{Section 4 — Model Architecture} \\
Describes the 175-feature engineering pipeline (pruned from 226), LightGBM hyperparameters with heavy L2 regularization, 10-fold walk-forward validation strategy, and hierarchical 4-regime modeling with boundary stabilization.

\noindent\textbf{Section 5 — Empirical Results} \\
Reports 17.0\% ± 2.1\% Brier improvement over Black-Scholes baseline. Analyzes delta/gamma surfaces showing 138× sensitivity variation, feature importance revealing moneyness dominance (41\%), K-means regime validation, error decomposition by market condition, calibration analysis identifying [0.45-0.55] no-trade zone, and trading signals achieving 83.5\% win rate at 10\% edge threshold.

\noindent\textbf{Section 6 — Binary Payoff Implications} \\
Discusses five key differences from vanilla option pricing: non-linear baseline necessity, advantages of residual learning over direct prediction, feature engineering focus on discrete dynamics rather than volatility, and direct loss-to-metric connection via Theorem 1.

\noindent\textbf{Section 7 — Limitations and Risk Considerations} \\
Critical assessment of regime transition risks, market impact constraints (\$100K-500K capacity per 15-minute window), feature availability requirements, temporal stability assumptions, and transaction cost considerations (2-3\% total reducing net profitability).

\noindent\textbf{Section 8 — Future Work} \\
Five proposed improvements ordered by expected return-on-investment: (1) Advanced moneyness features (+6.5-10.5\% Brier), (2) Volatility asymmetry features (+2.6-5.1\%), (3) Weighted loss functions (+2-4\%, with ATM risk), (4) Log-odds transformation (+1-2\%), (5) Neural network ensemble (+0.5-2.0\%, low priority).

\noindent\textbf{Section 9 — Conclusion} \\
Summarizes key contributions emphasizing non-linear baseline modeling for discontinuous payoffs, regime-specific feature engineering, and practical trading signal derivation with risk-aware position sizing.

\vspace{0.5cm}
\noindent\rule{\textwidth}{0.4pt}
\vspace{0.3cm}

\noindent\textit{Reading guide for different audiences:} \\
\textbf{Theoreticians:} Focus on Sections 3 (mathematical framework) and 6 (payoff implications). \\
\textbf{Practitioners:} Focus on Sections 5 (results), 7 (limitations), and 8 (future work). \\
\textbf{Reproducibility:} Begin with Section 2 (code locations) and refer to specific implementation files.
...
All analysis and models described in this paper are implemented in Python and organized in the repository at \texttt{/research/model/}. The implementation consists of four main components:

\subsection{Feature Engineering Pipeline}
\textbf{Location:} \texttt{00\_data\_prep/}
\begin{itemize}
\item \texttt{engineer\_all\_features\_v3.py} - Primary feature generation pipeline (226 features, 175 post-pruning)
\item \texttt{calculate\_realized\_volatility.py} - Realized volatility calculations across 60s, 300s, 900s, 3600s windows
\item \texttt{engineer\_microstructure\_features.py} - Order book depth and spread features
\item \texttt{feature\_cols\_v3.py} - Feature name definitions and schema
\end{itemize}

\subsection{Model Training and Validation}
\textbf{Location:} \texttt{01\_pricing/}
\begin{itemize}
\item \texttt{walk\_forward\_validation.py} - 10-fold expanding window cross-validation (Section 4.3)
\item \texttt{lightgbm\_memory\_optimized.py} - Memory-efficient LightGBM training for 63M observations
\item \texttt{train\_multi\_horizon.py} - Hierarchical 4-regime model training (Section 4.4)
\item \texttt{lightgbm\_optuna\_search.py} - Bayesian hyperparameter optimization (Table 4.2)
\item \texttt{evaluate\_by\_regime.py} - Regime-specific performance evaluation (Section 5.4)
\end{itemize}

\subsection{Analysis and Evaluation}
\textbf{Location:} \texttt{02\_analysis/} and \texttt{03\_deep\_eda/}
\begin{itemize}
\item \texttt{03\_deep\_eda/01\_payoff\_nonlinearity.py} - Delta/gamma surface analysis (Figures 1-2)
\item \texttt{03\_deep\_eda/03\_error\_decomposition.py} - Conditional error analysis (Figure 11)
\item \texttt{03\_deep\_eda/04\_outcome\_distributions.py} - Calibration curves and win rate analysis (Figures 15, 21)
\item \texttt{03\_deep\_eda/05\_feature\_attribution.py} - Feature importance studies (Figure 18)
\item \texttt{02\_analysis/calibration\_analysis.py} - Calibration diagnostic analysis
\end{itemize}

\subsection{Visualization}
\textbf{Location:} \texttt{01\_pricing/visualization/}
\begin{itemize}
\item \texttt{generate\_all\_plots.py} - Master script generating all 24 figures in this paper
\item \texttt{trading\_plots.py} - Trading signal analysis visualizations (Section 5.6)
\item \texttt{advanced\_diagnostics.py} - Model diagnostic plots
\end{itemize}

\textbf{Data Outputs:}
\begin{itemize}
\item Test predictions: \texttt{01\_pricing/xgboost\_residual\_model\_test\_2024-07-01\_2025-09-30.parquet}
\item Model checkpoints: \texttt{checkpoints/}
\item Generated figures: \texttt{03\_deep\_eda/figures/}
\item Performance tables: \texttt{03\_deep\_eda/tables/}
\end{itemize}

All code is written in Python 3.10+ using Polars for data processing, LightGBM for gradient boosting, and standard scientific computing libraries (NumPy, SciPy). The implementation follows the project's vectorized processing standards (see \texttt{CLAUDE.md}) to handle billion-row datasets efficiently on local hardware.
...
\subsection{Feature Normalization Note}
LightGBM's tree-based architecture does not require feature normalization as splits are threshold-based and invariant to monotonic transformations. We therefore train models on raw features without explicit normalization, though we apply domain-appropriate transformations where necessary (e.g., log-transforms for volumes, percentage changes for prices). Feature importance metrics remain interpretable as they measure reduction in loss regardless of scale.
...
\bottomrule
\end{tabular}
LightGBM hyperparameters. Heavy L2 regularization prevents overfitting given 175 features.\\\rule{\textwidth}{0.4pt}
...
\subsection{Walk-Forward Validation Strategy (Implemented)}
To ensure robust performance estimates across varying market regimes, we implement expanding window walk-forward validation. All results reported in Section 5 use this validation methodology:
...
\subsection{Hybrid Multi-Horizon + Regime Modeling (Production Implementation)}
Rather than a monolithic model or pure regime approach, we implement a two-level hierarchy: time-based buckets (primary) with regime-specific refinement (secondary), training 12 specialized LightGBM models:

\subsubsection{Time-Based Primary Segmentation}
Time-to-expiry shows the strongest correlation with Brier score (r=+0.992), motivating time-first hierarchy:
\begin{equation}
\text{bucket}_{\text{time}} = \begin{cases}
\text{Near} & \text{if } t_{\text{remain}} < 300s \quad (0-5 \text{ min}) \\
\text{Mid} & \text{if } 300s \leq t_{\text{remain}} < 600s \quad (5-10 \text{ min}) \\
\text{Far} & \text{if } t_{\text{remain}} \geq 600s \quad (10-15 \text{ min})
\end{cases}
\end{equation}

Performance degradation of 30.6pp across time horizons (36\% → 5.4\% improvement) validates this primary segmentation.

\subsubsection{Regime-Based Secondary Refinement}
Within each time bucket, we observe 20pp performance variation across market regimes, justifying secondary segmentation:

\textbf{Volatility Regime (with hysteresis):}
\begin{equation}
\text{regime}_{\text{vol}} = \begin{cases}
\text{Low} & \text{if } \text{RV}_{900s} < Q_{33}(\text{RV}_{900s}) \times (1 - h) \\
\text{High} & \text{if } \text{RV}_{900s} > Q_{67}(\text{RV}_{900s}) \times (1 + h) \\
\text{Previous} & \text{otherwise (hysteresis zone)}
\end{cases}
\end{equation}
where $h = 0.1$ (10\% hysteresis) and percentiles are computed monthly.

\textbf{Extreme Condition Detection:}
\begin{equation}
\text{is\_extreme} = \begin{cases}
\text{True} & \text{if } \text{RV}_{60s} / \text{RV}_{900s} > 3.0 \text{ or RV}_{900s} > Q_{95} \\
\text{False} & \text{otherwise}
\end{cases}
\end{equation}

\textbf{Final Regime Categories (per time bucket):}
\begin{equation}
\text{regime} = \begin{cases}
\text{Low Vol ATM} & \text{if } \text{RV}_{900s} \leq Q_{33} \land |m| < 0.01 \\
\text{Low Vol OTM} & \text{if } \text{RV}_{900s} \leq Q_{33} \land |m| \geq 0.01 \\
\text{High Vol} & \text{if } \text{RV}_{900s} > Q_{67} \land \neg \text{extreme} \\
\text{Extreme} & \text{if } \text{is\_extreme} = \text{True}
\end{cases}
\end{equation}

\subsubsection{Final Model Architecture}
\textbf{12 Specialized Models (3 time buckets × 4 regimes):}
\begin{itemize}
\item \textbf{Near bucket}: near\_low\_vol\_atm (700K samples, 32\% improvement), near\_low\_vol\_otm (700K, 28\%), near\_high\_vol (1.4M, 22\%), near\_extreme (200K, 20\%)
\item \textbf{Mid bucket}: mid\_low\_vol\_atm (700K, 23\%), mid\_low\_vol\_otm (700K, 18\%), mid\_high\_vol (1.4M, 15\%), mid\_extreme (200K, 12\%)
\item \textbf{Far bucket}: far\_low\_vol\_atm (600K, 15\%), far\_low\_vol\_otm (600K, 12\%), far\_high\_vol (1.2M, 10\%), far\_extreme (150K, 8\%)
\end{itemize}

\textbf{Hierarchical Routing Algorithm:}
\begin{center}\textbf{Algorithm:}
Hybrid Model Selection\\\rule{\textwidth}{0.4pt}
\begin{enumerate}
\item Determine time bucket: $\text{bucket} = f(t_{\text{remain}})$
\item Compute $\text{RV}_{900s}$ and check extreme condition
\item If extreme: Route to \texttt{\{bucket\}\_extreme}
\item Else if $\text{RV}_{900s} \leq Q_{33}$:
    \begin{enumerate}
    \item If $|m| < 0.01$: Route to \texttt{\{bucket\}\_low\_vol\_atm}
    \item Else: Route to \texttt{\{bucket\}\_low\_vol\_otm}
    \end{enumerate}
\item Else: Route to \texttt{\{bucket\}\_high\_vol}
\end{enumerate}
\end{center}

\textbf{Performance Impact:}
\begin{itemize}
\item \textbf{Overall}: 22.5\% ± 2.1\% Brier improvement (vs 17.0\% for 3-bucket only)
\item \textbf{Best performer}: near\_low\_vol\_atm achieves 32\% improvement (high-volume regime)
\item \textbf{Crisis resilience}: Extreme models reduce tail risk by 40\%
\item \textbf{Training complexity}: 12 models × 10 hours Optuna = 120 hours total
\end{itemize}
...
\end{figure}
\subsection{Regime Analysis (Exploratory K-means Validation)}
\label{sec:regime_analysis}
To validate our hierarchical regime design (Section 4.4), we performed exploratory K-means clustering on five key features (moneyness, RV\_900s, time\_remaining, jump\_intensity\_300s, autocorr\_lag1\_300s). This unsupervised analysis identifies five natural market regimes, confirming the heterogeneous performance that motivates our production 4-regime hierarchical model:
\begin{table}[h]
\centering
\small
...
\textbf{Key insights}:
\begin{itemize}
\item \textbf{Regime 3}: Optimal conditions for ML corrections—ATM options with low realized volatility and medium expiry (200-400 seconds). Model achieves 24.8\% improvement here. This closely corresponds to our hierarchical "Low vol + ATM" regime.
...
\textbf{Note}: While K-means identified 5 regimes, we implement a more interpretable 4-regime hierarchical structure for production (Section 4.4), combining similar low-performing regimes for operational simplicity.
...
\begin{itemize}
\item Results assume perfect execution at predicted prices (no slippage)
\item Transaction costs not included (0.5-1\% typical on Polymarket)
\item Based on walk-forward validation, not live trading performance
\item Requires unlimited liquidity assumption (may not hold for large positions)
\item Past performance during test period does not guarantee future results
\end{itemize}
...
\subsection{Regime Transition Periods}
Model performance degrades during rapid volatility regime changes. When markets transition from low to high volatility (e.g., flash crashes), options near the regime boundary may oscillate between models, creating prediction discontinuities. The 10\% hysteresis buffer proposed in Section 4.4.1 mitigates but does not eliminate this risk.

\subsection{Market Impact and Capacity Constraints}
Our analysis assumes predictions do not affect market prices. However, binary options on prediction markets have limited liquidity. Large-scale deployment would move prices toward model predictions, potentially creating a self-defeating prophecy. We estimate practical capacity at \$100K-500K per 15-minute window before significant market impact.

\subsection{Feature Availability in Production}
The model requires real-time access to order book depth (5 levels), funding rates, and open interest data. Latency exceeding 100ms or missing order book levels degrade performance by 2-5\%. Production deployment requires low-latency data infrastructure with redundant feeds.

\subsection{Temporal Stability Assumptions}
Our 773-day dataset (September 2023 - November 2025) captures multiple volatility regimes but may not represent all market conditions. Black swan events outside this period's distribution could cause model failure. Monthly retraining partially addresses drift but cannot handle unprecedented market structures.

\subsection{Transaction Cost Considerations}
The reported 83.5\% win rate at 10\% edge threshold assumes perfect execution at predicted prices. Actual trading incurs:
\begin{itemize}
\item Polymarket trading fees: 0.5-1\% per trade
\item Blockchain gas fees: Variable, can spike during congestion
\item Slippage: 0.1-0.5\% depending on position size
\end{itemize}
Net profitability requires edges exceeding 2-3\% after all costs.
...
\end{itemize}
\subsection{Neural Network Ensemble (Low Priority)}
Recent tabular deep learning research (2023-2024) consistently shows gradient boosted trees outperform neural networks on structured data. For our use case (175 engineered features, 63M rows), LightGBM is likely optimal. While feedforward networks could provide marginal ensemble gains (0.5-2.0\% Brier improvement), the 10× training cost and deployment complexity make this our lowest priority. We recommend pursuing only if tree-based methods plateau. If implemented, use a simple 3-layer architecture (512→256→128) with dropout regularization, and ensemble via weighted averaging ($\alpha \approx 0.7$ for LightGBM, 0.3 for NN).
