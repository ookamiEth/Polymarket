...
def consolidate_csv_files_batched(
    input_dir: str,
    output_path: str,
    batch_size: int = 30,
    checkpoint_dir: str = "checkpoints_consolidation",
) -> pl.DataFrame:
    """Consolidate all CSV.gz files into single sorted Parquet using batch processing.
    ...
    Args:
        ...
        checkpoint_dir: Directory for intermediate batch files

    Returns:
        Consolidated DataFrame
    """
    logger.info("=" * 80)
    logger.info("STAGE 1: CONSOLIDATING CSV.GZ FILES (BATCHED)")
    logger.info("=" * 80)
    ...
    logger.info(f"Intermediate files will be stored in: {checkpoint_dir}")
    ...
    for batch_idx in range(num_batches):
        ...
        batch_file_path = checkpoint_path / f"batch_{batch_idx:04d}.parquet"
        ...
        # Combine and sort batch
        logger.info(f"  Combining {len(lazy_dfs)} files...")
        batch_combined = pl.concat(lazy_dfs)

        logger.info("  Sorting by timestamp...")
        batch_start = time.time()
        batch_df = batch_combined.sort("timestamp").collect()
        batch_elapsed = time.time() - batch_start

        logger.info(f"  Collected {len(batch_df):,} rows in {batch_elapsed:.1f}s")

        # Deduplicate within batch
        batch_df = batch_df.unique()
        logger.info(f"  After deduplication: {len(batch_df):,} rows")

        # Write batch to intermediate Parquet
        logger.info(f"  Writing {batch_file_path.name}...")
        batch_df.write_parquet(batch_file_path, compression="snappy")
    ...
    logger.info("=" * 80)
    logger.info("MERGING SORTED BATCHES")
    logger.info("=" * 80)
    logger.info(f"Merging {len(batch_files)} batch files...")

    # Use lazy evaluation to merge without loading everything into memory
    merge_start = time.time()

    # Scan all batch Parquets lazily
    lazy_batches = [pl.scan_parquet(batch_file) for batch_file in batch_files]

    # Concatenate and sort (Polars will optimize this)
    merged = pl.concat(lazy_batches).sort("timestamp")
    ...
    logger.info(f"Merged {len(df):,} rows in {merge_elapsed:.1f}s")

    # Final deduplication (in case duplicates span batch boundaries)
    ...
    # Write final consolidated Parquet
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Consolidate Deribit options quote CSV.gz files and sample to 1-second granularity with forward-fill (BATCHED VERSION)"
    )
    ...
    parser.add_argument(
        "--checkpoint-dir",
        default="checkpoints_consolidation",
        help="Directory for intermediate batch files (default: checkpoints_consolidation)",
    )
    parser.add_argument(
        "--max-fill-gap",
        type=int,
    ...
    logger.info("=" * 80)
    logger.info("CONSOLIDATE & SAMPLE OPTIONS QUOTES (BATCHED)")
    logger.info("=" * 80)
    ...
    if args.skip_consolidation:
        ...
        df = consolidate_csv_files_batched(
            args.input_dir,
