#!/usr/bin/env python3
"""
Download historical orderbook snapshot data (top 25 levels) for Binance markets.

This script downloads book_snapshot_25 data containing:
...
Usage:
    uv run python download_binance_orderbook_25.py \\
...
Output: data/raw/binance_orderbook_25/{exchange}/{symbol}/{date}.parquet
"""
...
def _empty_dataframe() -> pl.DataFrame:
    """Create empty DataFrame with book_snapshot_25 schema."""
    schema = {
        "exchange": pl.Utf8,
        "symbol": pl.Utf8,
...
def download_orderbook_25_for_day(
    date_str: str,
    exchange: str,
    symbols: list[str],
    api_key: str,
    ...
    """Download book_snapshot_25 data for single day.
    ...
    """
    logger.debug(f"Downloading {exchange} orderbook_25 for {date_str} ({', '.join(symbols)})...")
    ...
    try:
        ...
        for symbol in symbols:
            filename = f"{exchange}_book_snapshot_25_{date_str}_{symbol}.csv.gz"
...
def parse_book_snapshot_csv(csv_file: str) -> pl.DataFrame:
    """Parse book_snapshot_25 CSV to standardized schema.
...
def process_day(
    ...
    for symbol in symbols:
        ...
        try:
            # Download
            csv_files = download_orderbook_25_for_day(date_str, exchange, [symbol], api_key, temp_dir)
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description="Download Binance orderbook snapshot data (top 25 levels) from Tardis",
...
Examples:
  ...
  uv run python download_binance_orderbook_25.py \\
    ...
    parser.add_argument(
        "--output-dir",
        default="./data/raw/binance_orderbook_25",
        help="Output directory (default: ./data/raw/binance_orderbook_25)",
    )
    parser.add_argument(
        "--temp-dir",
    ...
    checkpoint_file = os.path.join(args.checkpoint_dir, "binance_orderbook_25_progress.json")
    completed_dates = set()
