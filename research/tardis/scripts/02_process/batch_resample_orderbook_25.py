#!/usr/bin/env python3
"""
Batch resample Binance orderbook snapshot (top 25 levels) files to 1-second intervals.

Automatically discovers and processes all orderbook snapshot files from raw data directory.
Supports parallel processing and checkpointing for large-scale resampling.

Usage:
    # Process all files
    uv run python batch_resample_orderbook_25.py \
...
from resample_orderbook_25_to_1s import resample_file
...
def discover_input_files(input_dir: str) -> list[dict]:
    ...
    Args:
        input_dir: Root directory containing orderbook snapshot files
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description="Batch resample Binance orderbook snapshot (top 25 levels) data to 1-second intervals",
...
Examples:
  ...
  uv run python batch_resample_orderbook_25.py \
    ...
    parser.add_argument(
        "--input-dir",
        required=True,
        help="Input directory containing raw orderbook snapshot data",
    )
    parser.add_argument(
        "--output-dir",
        required=True,
    ...
    checkpoint_file = os.path.join(args.checkpoint_dir, "batch_orderbook_25_resample_progress.json")
    completed_files = set()
