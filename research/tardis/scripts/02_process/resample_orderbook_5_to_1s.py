#!/usr/bin/env python3
"""
Resample Binance orderbook snapshot (top 5 levels) data to 1-second intervals.
...
Usage:
    uv run python resample_orderbook_5_to_1s.py \
...
def resample_to_1s_last(df: pl.DataFrame) -> pl.DataFrame:
    ...
    # Generate list of all price level columns (10 ask + 10 bid)
    price_level_cols = []
    for i in range(5):
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description="Resample Binance orderbook snapshot (top 5 levels) data to 1-second intervals",
