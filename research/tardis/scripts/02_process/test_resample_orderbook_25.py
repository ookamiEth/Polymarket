#!/usr/bin/env python3
"""
Test script for orderbook snapshot (top 25 levels) resampling functionality.
...
Usage:
    uv run python test_resample_orderbook_25.py

Requirements:
    - Downloaded data in data/raw/binance_orderbook_25/
...
def test_resample_basic() -> None:
    ...
    input_dir = Path("data/raw/binance_orderbook_25/binance-futures/BTCUSDT")
    if not input_dir.exists():
        logger.error(f"Input directory not found: {input_dir}")
        logger.error("Please download sample data first using download_binance_orderbook_25.py")
        sys.exit(1)
...
def main() -> None:
    ...
    try:
        ...
        logger.info("")
        logger.info("uv run python batch_resample_orderbook_25.py \\")
        logger.info("    --input-dir data/raw/binance_orderbook_25 \\")
        logger.info("    --output-dir data/processed/binance_orderbook_25_1s \\")
